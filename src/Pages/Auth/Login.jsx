...
// data ya tendrá { username, password } desde el formulario.
...
try {
    ...
    // Llamada al backend para loguear (serviceLogin debe estar configurado de forma similar a serviceRegister)
    const response = await serviceRegister.enter(data);
    // Opcional: Si el backend retorna token u otros datos, guárdalos (por ejemplo en localStorage)
        ...
        <div className='w-full h-full flex flex-col justify-center items-center bg-white p-5 rounded-lg shadow-lg'>
            ...
            <Form className='self-start w-full flex flex-col gap-4' onFinish={handleSubmit}>
                <div>
                    ...
                    <Input value={login.username} placeholder='correo electronico' size="large" 
                    onChange={(e) => setLogin({...login, username: e.target.value})}/>
                </div>
                <div>
                    <Text type="secondary">Password</Text>
                    <Input.Password value={login.password} placeholder='contraseña' size="large" 
